...
suitable for :func:`hoct_inference.predict`.

Image reading uses ``bioio`` (optional dependency, ``hoct-inference[bioio]``).
"""
...
def _require_bioio():
    ...
    try:
        ...
        raise ImportError(
            "Reading raw images requires the 'bioio' extra. Install it with: pip install 'hoct-inference[bioio]'"
        ) from exc
    return BioImage
